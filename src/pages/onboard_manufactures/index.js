...
export const checkNumbervalue = (event) => {
  ...
  if (!/^[0-9]*\.?[0-9]*$/.test(event.key) && event.key !== "Backspace" || event.key === ".") {
    return true;
  }
};
...
export const checknumcharCharacter = (event) => {
  if (
    /^[A-Za-z]{500}0[A-Z0-9a-z]{7}$/.test(event.key) &&
    event.key !== "Backspace"
  ) {
    return true;
  }
...
export const checkAlphabetsNumbersAmpersand = (event) => {
  if (!/^[a-zA-Z0-9&\.,]*$/.test(event.key) && event.key !== "Backspace") {
    return true;
  }
};
